...
Run once to add the new columns:
    python -m cortex.migrate_usernames

This only adds the columns — it does NOT backfill values.
Use `python -m cortex.set_usernames auto` to backfill usernames,
and `python -m cortex.set_usernames slugify-projects` to backfill project slugs.
"""
...
from cortex.config import DB_FILE
...
def migrate():
    ...
    if changes:
        ...
        print("\nMigration complete.")
        print("Next steps:")
        print("  python -m cortex.set_usernames auto           # backfill usernames")
        print("  python -m cortex.set_usernames slugify-projects  # backfill project slugs")
    else:
