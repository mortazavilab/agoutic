...
from cortex.dataframe_sources import (
    ...
    load_full_parsed_dataframe_locally as load_full_parsed_dataframe_locally_shared,
)
...
async def _rehydrate_truncated_plot_sources(
    ...
    relevant_df_ids = sorted({
        ...
        if isinstance(spec.get("df_id"), int)
    })
    ...
    for df_id in relevant_df_ids:
        ...
        if not full_result:
            _chart_types = {
                ...
                for spec in plot_specs
                if spec.get("df_id") == df_id
            }
            if _chart_types & {"venn", "upset"}:
                errors.append(
    ...
    *,
    ...
    if params:
        ...
        if isinstance(result, dict) and result.get("data"):
            if not is_truncated_dataframe_payload(result):
                return result
            logger.info(
                "Analyzer dataframe rehydration remained truncated; falling back to local reload",
                file_path=file_path,
                work_dir=work_dir,
                run_uuid=run_uuid,
                row_count=result.get("row_count"),
                preview_rows=len(result.get("data") or []),
            )
...
def _load_full_parsed_dataframe_locally(
    ...
    *,
    ...
    return load_full_parsed_dataframe_locally_shared(
        parse_source,
        payload=payload,
        project_dir_path=project_dir_path,
    )
    ...
    async def run(self, ctx: ChatContext) -> None:
        ...
        if ctx.plot_specs:
            ...
            for _df_key, _chart_group in _plots_by_df.items():
                ...
                _plot_candidates = [
                    (name, payload)
                    for name, payload in (plot_source_dataframes or {}).items()
                    if (payload.get("metadata") or {}).get("df_id") == _df_key
                ]
                if _plot_candidates:
                    _plot_label, _plot_payload = max(
                        _plot_candidates,
                        key=lambda item: (
                            int(item[1].get("row_count") or 0),
                            len(item[1].get("data") or []),
                        ),
                    )
                else:
                    _plot_payload = hydrate_dataframe_payload_locally(
                        _df_key,
                        ctx.history_blocks,
                        current_dataframes=plot_source_dataframes,
                        project_dir_path=ctx.project_dir_path,
                    )
                    _plot_label = (_plot_payload.get("metadata") or {}).get("label") if _plot_payload else None

                if _plot_payload:
                    _plot_dfs[_plot_label or f"DF{_df_key}"] = _plot_payload
