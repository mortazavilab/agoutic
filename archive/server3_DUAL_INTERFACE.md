# Server 3: Dual Interface Architecture (REST + MCP)

**Status:** Week 3 - Enhanced with MCP Protocol Support

## Overview

Server 3 now exposes **both** REST API and MCP (Model Context Protocol) interfaces, allowing flexible integration patterns:
...
**Location:** `server3/mcp_tools.py` + `server3/mcp_server.py`
...
# Keep running in background, or integrate into Server 1 process
```

### Call Tool from Server 1
...
The LLM in Server 1 can directly use these tools:
...
server1/
├── mcp_client.py         # MCP client for Server 1
└── [existing Agent code]
```
...
### MCP Client (`server1/mcp_client.py`)
...
### Integration Layer (`server1/mcp_client.py`)

`Server1AgentWithMCP` class:
...
### Option 2: Integrated in Server 1

```bash
# Server 1 starts both:
# 1. REST server on port 8001
# 2. MCP subprocess on-demand

# See: server1/agent_engine.py (future integration)
```
...
python -m server3.mcp_server 2>&1 | head -20
...
- **Integration:** See [server1/mcp_client.py](../server1/mcp_client.py)
