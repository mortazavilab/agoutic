...
- **Cross-workflow open-chromatin overlap requests can now run as monitored background script workflows** — Region-overlap venn/upset requests using workflow references now classify into a structured workflow that identifies BED files first, pauses for approval, runs an allowlisted overlap script in the background, writes the overlap CSV outputs into a fresh workflow folder under the active project, parses those CSVs back into workflow dataframes, and renders the venn/upset chart from the overlap membership table without blocking the UI.
  (`analyzer/analysis_engine.py`, `analyzer/mcp_tools.py`, `analyzer/mcp_server.py`, `cortex/chat_dataframes.py`, `cortex/data_call_generator.py`, `cortex/llm_validators.py`, `cortex/tool_dispatch.py`, `skills/run_dogme_dna/SKILL.md`, `tests/analyzer/test_mcp_tools.py`, `tests/cortex/test_chat_data_calls.py`)

- **Cross-project open-chromatin Venn requests now understand `project_slug:workflowN` shorthand and route through the result-analysis skills automatically** — prompts like "make a venn diagram of the regions in testslopenchrom:workflow2 and testopenchrom2:workflow4" now auto-route to `analyze_job_results`, normalize each shorthand reference to the workflow's `openChromatin/` folder, and default the overlap comparison to `*.m6Aopen.bed` with `1 bp` overlap semantics while the DNA/result-analysis skills explicitly document that shorthand workflow.
  (`cortex/llm_validators.py`, `cortex/data_call_generator.py`, `skills/analyze_job_results/SKILL.md`, `skills/run_dogme_dna/SKILL.md`, `tests/cortex/test_skill_detection.py`, `tests/cortex/test_pure_helpers2.py`, `tests/cortex/test_chat_data_calls.py`)
...
- **Heatmap, PCA, and publication-bar requests now share one routed plotting contract from chat through edgePython** — Cortex now keeps only simple exploratory bars on the interactive `[[PLOT:...]]` path, routes all heatmaps plus PCA and publication-style bar requests through edgePython `generate_plot`, resolves existing `DFN` chat dataframes to file-backed `input_path` sources before dispatch, and extends edgePython `generate_plot` so generic rehydrated tables can render heatmap, PCA, bar, and stacked_bar outputs without relying on DE state. The first-pass, second-pass, and planner prompts now mirror that contract so the model no longer promises declarative heatmaps or unsupported fallback paths.
  (`cortex/plot_routing.py`, `cortex/tag_parser.py`, `cortex/chat_dataframes.py`, `cortex/dataframe_sources.py`, `cortex/tool_dispatch.py`, `cortex/edgepython_plot_params.py`, `cortex/prompt_templates/first_pass_system_prompt.md`, `cortex/prompt_templates/second_pass_system_prompt.md`, `cortex/prompt_templates/planning_system_prompt.md`, `edgepython_mcp/edgepython_server.py`, `edgepython_mcp/tool_schemas.py`, `tests/cortex/test_de_routing.py`, `tests/cortex/test_chat_data_calls.py`, `tests/test_edgepython_plot_helpers.py`)

- **Interactive Plotly charts and edgePython artifacts now share AGOUTIC's light publication-style defaults** — saved matplotlib plots and inline Plotly charts now use aligned warm-light surfaces, darker axes, y-only grids, colorblind-safe categorical colors, gentler label-angle fallback, and clearer UpSet/bar/PCA/volcano styling so in-app charts and exported figures feel visually consistent while keeping correctness and readability ahead of aesthetics.
  (`edgepython_mcp/edgepython_server.py`, `ui/theme.py`, `ui/appui_renderers.py`, `cortex/prompt_templates/first_pass_system_prompt.md`, `cortex/prompt_templates/second_pass_system_prompt.md`, `skills/differential_expression/SKILL.md`, `docs/DATAFRAMES.md`, `SKILLS.md`, `tests/test_edgepython_plot_helpers.py`, `tests/ui/test_theme_helpers.py`, `tests/ui/test_app_source_helpers.py`)
...
- **DE interpretation now runs GO enrichment separately for up- and down-regulated genes and returns two workflow tables with GO namespace labels** — the `INTERPRET_RESULTS` step now builds distinct upregulated and downregulated GO enrichment dataframes, includes the enriched term namespace as `BP`, `MF`, or `CC`, preserves prose interpretation plus per-table summaries, and renders those workflow-step tables inline in the UI instead of raw JSON.
  (`analyzer/enrichment_engine.py`, `cortex/plan_executor.py`, `ui/appui_block_part2.py`, `ui/appUI.py`, `ui/appui_services.py`, `tests/cortex/test_plan_executor_run_script.py`, `tests/ui/test_app_source_helpers.py`)
