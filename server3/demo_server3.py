"""
Demo/Integration script for Server 3.
...
# Configuration
SERVER3_URL = "http://127.0.0.1:8001"
POLL_INTERVAL = 2  # seconds

class Server3Client:
    """Async client for Server 3 API."""
    
    def __init__(self, base_url: str = SERVER3_URL):
...
async def demo_submit_dna_job():
    ...
    try:
        ...
        print(f"   Make sure Server 3 is running: uvicorn server3.app:app --port 8001")
        return
...
async def demo_all():
    ...
    print("\n🚀 AGOUTIC Server 3 - Comprehensive Demo\n")
...
if __name__ == "__main__":
    print("""
╔════════════════════════════════════════════════════════════╗
║        AGOUTIC Server 3 - Integration Demo               ║
    ...
    if len(sys.argv) > 1:
        ...
        if command == "dna":
            ...
            if len(sys.argv) > 2:
                ...
                print("Usage: python demo_server3.py track <run_uuid>")
        elif command == "logs":
            if len(sys.argv) > 2:
                run_uuid = sys.argv[2]
                asyncio.run(demo_get_logs(run_uuid))
            else:
                print("Usage: python demo_server3.py logs <run_uuid>")
        elif command == "list":
