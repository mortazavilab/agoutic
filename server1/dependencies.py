...
from server1.models import User, ProjectAccess, Project
...
def require_run_uuid_access(run_uuid: str, user: User) -> None:
    ...
    """
    from server3.models import DogmeJob as S3DogmeJob

    session = SessionLocal()
    try:
        # Use the shared SQLite DB — Server 1, 3, 4 all share it
        ...
        with Session(engine) as db:
            job = db.execute(
                select(S3DogmeJob).where(S3DogmeJob.run_uuid == run_uuid)
            ).scalar_one_or_none()
