...
Usage:
    python -m server1.set_usernames list                          # list all users
    python -m server1.set_usernames set <email> <username>        # set username for one user
    python -m server1.set_usernames auto                          # auto-generate all missing usernames
    python -m server1.set_usernames slugify-projects              # auto-generate slugs for all projects
    python -m server1.set_usernames migrate-dirs                  # create new dir structure + move data
"""
...
from server1.config import AGOUTIC_DATA, DB_FILE
...
def _update_work_dir_paths(conn: sqlite3.Connection, old_prefix: str, new_prefix: str):
    ...
    # Check if dogme_jobs table exists (it's in the server3 DB, might be shared)
...
def main():
    ...
    if cmd == "list":
        ...
        if len(sys.argv) != 4:
            print("Usage: python -m server1.set_usernames set <email> <username>")
            sys.exit(1)
