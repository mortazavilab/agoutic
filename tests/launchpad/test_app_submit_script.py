...
async def test_submit_script_job_success(monkeypatch, tmp_path):
    ...
    async def fake_create_job(*_args, **_kwargs):
        create_job_kwargs.update(_kwargs)
        return fake_job
    ...
    assert create_job_kwargs["workflow_key"] == "dogme"
    assert subprocess_kwargs["start_new_session"] is True


@pytest.mark.asyncio
async def test_submit_script_job_preserves_explicit_workflow_key(monkeypatch, tmp_path):
    script_path = tmp_path / "run.py"
    script_path.write_text("print('ok')\n")
    create_job_kwargs = {}
    ...
    monkeypatch.setattr(launchpad_app, "create_job", fake_create_job)
    monkeypatch.setattr(launchpad_app, "add_log_entry", fake_add_log_entry)
    monkeypatch.setattr(launchpad_app.uuid, "uuid4", lambda: "run-reconcile")
    monkeypatch.setattr(
        launchpad_app,
    ...
    assert result["run_uuid"] == "run-reconcile"
    assert create_job_kwargs["workflow_key"] == "reconcile_bams"
